...
namespace SMS_Bomber_V2
{
    public partial class Form1 : RelyForm
    {
        ...
        private void CONFIG() //CONFIG
        {
            ...
            {
                SendError++;
                Error_SEND_SMS_LBL.Text = SendError.ToString();
            }
        }
        ...
        private void Form1_Load(object sender, EventArgs e) // LOAD FORM
        {
            ...
            try
            {
                ...
                var ipInfo = GetIP_info.Get("https://ipwhois.app/json/?lang=en");
                string IP_Info_Sou = ipInfo.ToString();

                Match Get_IP_Reg = Regex.Match(IP_Info_Sou, "currency_code\":\"(.*?)\"");
            ...
            try
            {
                ...
                var GetTest_Net = Test_Net.Get("https://www.google.com/").ToString();

                if (GetTest_Net.Contains("<title>Google</title>"))
                {
        ...
        {
            if(Mobile_TXT.Text == "" || !Mobile_TXT.Text.Contains("09") || Mobile_TXT.Text.Length != 11)
